...
public class Ex05 {
	...
	// 집합 s1과 s2의 합집합을 복사
	public void unionOf(Ex05 s1, Ex05 s2) {
	...
	// 집합 s의 부분집합인가요?
	public boolean isSubsetOf(Ex05 s) {
		for (int i = 0; i < num; i++) {
			int j = 0;
			for (; j < s.num; j++)
				if (set[i] == s.set[j])
	...
	// 집합 s의 진부분집합인가요?
	public boolean isProperSubsetOf(Ex05 s) {
	...
	// 집합 s1과 s2의 교집합을 복사
	public void intersectionOf(Ex05 s1, Ex05 s2) {
		clear();
	...
	// 집합 s1과 s2의 차집합을 복사
	public void differenceOf(Ex05 s1, Ex05 s2) {
		clear();
		for (int i = 0; i < s1.num; i++)
			if (!s2.contains(s1.set[i]))
				add(s1.set[i]);
